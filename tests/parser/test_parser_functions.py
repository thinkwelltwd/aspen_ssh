...
def test_take_pascal_bytestring():
    ...
    assert (b'fo\x00',
            b'\x01') == parser_helpers.take_pascal_bytestring(b'\x00\x00\x00\x03fo\x00\x01')
    with pytest.raises(InputTooShortError):
...
def test_take_pascal_string():
    ...
    assert ('fπø',
            b'\x00') == parser_helpers.take_pascal_string(b'\x00\x00\x00\x05f\xcf\x80\xc3\xb8\x00')


def test_take_list():
    assert ([1, 2], b'\x01') == parser_helpers.take_list(
        b'\x00\x00\x00\x08\x00\x00\x00\x01\x00\x00\x00\x02\x01', parser_helpers.take_u32
    )
